document.addEventListener('DOMContentLoaded',()=>{
    ...
    document.getElementById('bookingForm').addEventListener('submit',e=>{
        ...
        const all=JSON.parse(localStorage.getItem('lexxy_bookings')||'[]');all.push(d);localStorage.setItem('lexxy_bookings',JSON.stringify(all));
        e.target.reset();showToast('Booking request received. Our concierge will contact you shortly.');
    });
});
function bookSvc(s){
